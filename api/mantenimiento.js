...
/**
 * Aparatos con fichajes de más de una persona: el aviso de Telegram
 * (§ fichajes.js, avisarDispositivoCompartido) solo se manda la primera vez
 * que aparece cada pareja aparato+dos-nombres, así que no sirve para ver el
 * historial completo — esto sí: quién es el dueño habitual del aparato
 * (quien más lo usa) y, uno por uno, el día y la hora exactos en que
 * alguien más lo usó.
 *
...
export async function dispositivosCompartidos(db, centro) {
  ...
  const args = [];
  if (centro) {
    cond.push("LOWER(TRIM(COALESCE(centro,''))) = LOWER(TRIM(?))");
    args.push(centro);
  }

  const r = await db.execute({
    sql: `SELECT device_id, centro, empleado, tipo, fecha, hora, timestamp
          FROM fichajes WHERE ${cond.join(' AND ')}
          ORDER BY timestamp ASC`,
    args,
  });

  const confianzaPorCentro = {};
  async function esConfianza(c, id) {
    if (!(c in confianzaPorCentro)) {
      const cfg = await getCentroCfg(db, c);
      confianzaPorCentro[c] = new Set(
        String(cfg?.dispositivos_confianza || '').split(',').map(x => x.trim()).filter(Boolean)
      );
    }
    return confianzaPorCentro[c].has(id);
  }

  const porDispositivo = {};
  for (const fila of r.rows) {
    const clave = `${fila.centro || ''} ${fila.device_id}`;
    (porDispositivo[clave] ||= { device_id: fila.device_id, centro: fila.centro || '', eventos: [] })
      .eventos.push({
        empleado: fila.empleado, tipo: fila.tipo, fecha: fila.fecha,
        hora: String(fila.hora || '').slice(0, 5), timestamp: Number(fila.timestamp),
      });
  }

  const resultado = [];
  for (const grupo of Object.values(porDispositivo)) {
    const nombreCanonico = {};
    for (const e of grupo.eventos) {
      const clave = e.empleado.trim().toLowerCase();
      if (!(clave in nombreCanonico)) nombreCanonico[clave] = e.empleado;
    }
    if (Object.keys(nombreCanonico).length < 2) continue;
    if (await esConfianza(grupo.centro, grupo.device_id)) continue;

    const porPersona = {};
    for (const e of grupo.eventos) {
      const clave = e.empleado.trim().toLowerCase();
      const acc = porPersona[clave] ||= { empleado: nombreCanonico[clave], veces: 0, primero: e.timestamp, ultimo: e.timestamp };
      acc.veces++;
      acc.primero = Math.min(acc.primero, e.timestamp);
      acc.ultimo = Math.max(acc.ultimo, e.timestamp);
    }
    const porUso = Object.values(porPersona).sort((a, b) => b.veces - a.veces || a.primero - b.primero);
    const propietario = porUso[0].empleado;
    const claveDueno = propietario.trim().toLowerCase();

    const prestados = grupo.eventos
      .filter(e => e.empleado.trim().toLowerCase() !== claveDueno)
      .sort((a, b) => b.timestamp - a.timestamp)
      .map(e => ({ empleado: e.empleado, tipo: e.tipo, fecha: e.fecha, hora: e.hora, timestamp: e.timestamp }));

    resultado.push({
      device_id: grupo.device_id,
      centro: grupo.centro,
      propietario,
      total_propietario: porUso[0].veces,
      prestados,
      ultimo_uso: Math.max(...grupo.eventos.map(e => e.timestamp)),
    });
  }

  resultado.sort((a, b) => b.ultimo_uso - a.ultimo_uso);
  return resultado;
}
...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'GET' && accion === 'dispositivos-compartidos') {
      return res.status(200).json(await dispositivosCompartidos(db, req.query.centro || ''));
    }
